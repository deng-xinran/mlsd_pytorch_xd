...
#             batch_outputs[:, 1:5, :,: ] = label[:, 1:5, :,: ]
        ...
        self.writer.add_scalar('val/loss', average_loss, self.global_step)#log the validation loss
        return {
            'fscore': f_score,
            'recall': recall,
            'precision':precision,
            'sAP10': sAP,
            'val_loss':average_loss #include the validation loss in the return dictionary
        }
